...
class Tweet {
    ...
    get activityType() {
        ...
        if (indexOfDistanceKm != -1)
            start = indexOfDistanceKm;
        else if (indexOfDistanceMi != -1)
            start = indexOfDistanceMi;
        if (indexOfDash != -1)
            end = indexOfDash;
        else if (indexOfWith != -1)
            end = indexOfWith;
        ...
        if (activity.search("posted") != -1) {
            ...
            if (indexOfA != -1)
                activity = activity.substring(indexOfA + 3, indexOfIn);
            if (indexOfAn != -1)
                activity = activity.substring(indexOfAn + 4, indexOfIn);
        }
        //remove invalid activity
        if (activity.search("http") != -1 || activity.search(" - ") != -1)
            activity = "unknown";
        return activity;
    }
    get distance() {
        ...
        if (indexOfDistanceMi != -1)
            distance = parseFloat(this.text.substring(indexOfA + 2, indexOfDistanceMi - 1));
        return distance;
    }
